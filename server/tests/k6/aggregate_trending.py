...
# Trending files have "trending" in their filename
file_pattern = os.path.join(results_dir, "trending*.json")
files = glob.glob(file_pattern)

if not files:
    print(f"No trending results files found in '{results_dir}' matching 'trending*.json'.")
    exit(1)
...
print("\nOverall Aggregated Trending Summary:")
print(json.dumps(final_summary, indent=2))

# Write final aggregated summary to file
final_summary_file = os.path.join(results_dir, "final_trending_summary.json")
with open(final_summary_file, "w") as outfile:
    json.dump(final_summary, outfile, indent=2)

print(f"\nFinal trending aggregated summary written to {final_summary_file}")
